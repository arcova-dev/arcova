...
export function EmptyState({title,detail,action,href}: {title:string;detail:string;action?:string;href?:string}) { const btn=<Button variant="outline"><RefreshCw/>{action||"RETRY"}</Button>; return <div className="empty-state"><DatabaseZap/><span className="status unavailable">○ UNAVAILABLE</span><h2>{title}</h2><p>{detail}</p>{href?<Button asChild variant="outline"><Link to={href as "/markets"|"/agent"}>{action}<ArrowRight/></Link></Button>:btn}</div> }
