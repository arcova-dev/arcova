...
import { Menu, Radio, LogOut, UserRound } from "lucide-react";
...
export function ArcovaShell({ children }: { children: ReactNode }) {
 ...
 const [contract,setContract]=useState(false); const [email,setEmail]=useState<string|null>(null); const [network,setNetwork]=useState("UNAVAILABLE");
 useEffect(()=>{ void supabase.auth.getUser().then(({data})=>setEmail(data.user?.email??null)); void fetch("/api/public/network-status").then(r=>r.json()).then((v:{status?:string})=>setNetwork(v.status?.toUpperCase()??"UNAVAILABLE")).catch(()=>setNetwork("OFFLINE")); const {data}=supabase.auth.onAuthStateChange((_e,s)=>setEmail(s?.user.email??null)); return()=>data.subscription.unsubscribe();},[]);
